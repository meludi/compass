...
- **`HANDBOOK.md` names the silent PR-review failure** — a workflow that runs green and posts nothing looks identical to one waiting for an `@claude` mention, and only the second was documented. The cause is `permissions:` in `claude-code-review.yml`: with `pull-requests: read` the run succeeds and the comments are buffered and dropped, visible in the Actions log and nowhere else. Found while testing `/install-github-app` end to end.
