...
Can be triggered from `/ship` (step 5) or run standalone at any time.
...
- After `/ship` opens a PR and you answer "yes" to the review prompt
- Before `/ship`: review local changes early, before pushing
...
If the diff touches any of the following — run `/security-review` on those files:
