...
Run `/validate` (lint + types + tests + browser smoke). Fixes can break lint/types/tests — this is the gate. If validation fails, fix and re-run before finishing.
...
- **Never auto-commit** — stop after `/validate`; the author commits and pushes. (`/auto-implement` is the only sanctioned auto-commit exception.)
