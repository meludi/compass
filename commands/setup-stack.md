---
description: Scaffold the tech stack for a greenfield project — runs once, before the first /compass:worktree
argument-hint: [path to .work/prds/*.prd.md]
---

# /compass:setup-stack — Tech Stack Setup

> **Model:** `/model sonnet` — balanced model for this command.

Set up the tech stack for a new greenfield project. Run **once**, directly after `/compass:ideate` and before `/compass:create-stories`.
...
```
This project already has source files. /compass:setup-stack is designed for greenfield
...
If `package.json` is missing: stop and tell the user to resolve the scaffold failure, then re-run `/compass:setup-stack`.
...
The first three become the Mirror source for /compass:plan-feature.
...
The first three seed files are your Mirror source for /compass:plan-feature.
```

**Commit checkpoint:** the scaffold is a consistent, describable unit ("scaffold {framework} stack with welcome screen and smoke test"). Suggest committing it before the first feature: `Run /compass:commit to save the scaffold, then /compass:worktree <first-story-name>?` Suggest only; never commit without confirmation.
