...
If you have not reviewed the plan yet, use `/compass:implement` followed by `/compass:ship` instead. Auto-implementing an unreviewed plan is dangerous.
...
For everything else, stay with `/compass:implement` → `/compass:ship`.
...
Execute Steps 1–5 from `${CLAUDE_PLUGIN_ROOT}/commands/implement.md`:
...
If `${CLAUDE_PLUGIN_ROOT}/templates/husky-pre-commit.sh` has been installed as `.husky/pre-commit`, it runs on the auto-commit in Phase 2:
...
- Net effect: `/compass:auto-implement` and the husky hook compose cleanly — the hook acts as a final, non-blocking sanity check on top of Phase 1 validation.
