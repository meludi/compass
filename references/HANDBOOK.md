...
| The review workflow burns its budget and gives up | The bundled `code-review` plugin tells the agent to "launch a haiku agent". `haiku` is a model, not a subagent type, so the call fails, the retries eat the turn budget, and the run ends before anything is posted. Not a version you can update past: the workflow reinstalls the plugin from upstream `HEAD` on every run, and the line is still there ([anthropics/claude-code#38964](https://github.com/anthropics/claude-code/issues/38964) — closed by a stale bot, not by a fix; the community PR was never merged). Until it lands, replace the `/code-review` invocation in `claude-code-review.yml` with a direct review prompt. **Re-check before trusting this row** — it describes someone else's open defect. |
