...
```
/compass:worktree <name>          → creates directory + branch + installs deps
  └─ open claude session  → run /compass:plan-feature to load context and plan, then implement
...
|---|---|
| `bash ${CLAUDE_PLUGIN_ROOT}/scripts/worktree.sh <name> open` | Create worktree, install deps, open Claude |
| `bash ${CLAUDE_PLUGIN_ROOT}/scripts/worktree.sh <name> rm` | Guarded removal: dir + branch + prune. Refuses on uncommitted/unmerged work — add `-f`/`--force` to override |
